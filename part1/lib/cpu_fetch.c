...
void fetch_data() {
    ...
    switch (ctx.cur_inst->mode) {
        ...
        case AM_R_D8:
            ...
            emu_cycles(1); // 1 cpu cycle to read the bus
            ctx.regs.pc++;
            return;
        ...
        case AM_D16:
            // ctx.fetched_data = bus_read(ctx.regs.pc);
            // emu_cycles(1);
            // ctx.fetched_data |= bus_read(ctx.regs.pc + 1) << 8;
            // emu_cycles(1);
            // ctx.regs.pc += 2;
            // return;
            {
        ...
        case AM_R_HLI:
            // load from address in HL, then increment HL
        ...
        case AM_R_HLD:
            // load from address in HL, then decrement HL
            ctx.fetched_data = bus_read(cpu_read_reg(ctx.cur_inst->reg_2));
            emu_cycles(1);
            cpu_set_reg(RT_HL, cpu_read_reg(RT_HL) - 1);
            return;
        ...
        case AM_HLI_R:
            // store data in HL to R, then increment HL
            ctx.fetched_data = cpu_read_reg(ctx.cur_inst->reg_2);
            ctx.mem_dest = cpu_read_reg(ctx.cur_inst->reg_1);
            ctx.dest_is_mem = true;
            cpu_set_reg(RT_HL, cpu_read_reg(RT_HL) + 1);
        ...
        case AM_HLD_R:
            // store data in HL to R, then decrement HL
            ctx.fetched_data = cpu_read_reg(ctx.cur_inst->reg_2);
            ctx.mem_dest = cpu_read_reg(ctx.cur_inst->reg_1);
            ctx.dest_is_mem = true;
            cpu_set_reg(RT_HL, cpu_read_reg(RT_HL) - 1);
